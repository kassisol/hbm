...
func NewApi(version, appPath string) (*Api, error) {
	...
	uris.Register("GET", `^/containers/(.+)/resize`, AllowTrue, "container_resize", "resize", "Resize a container TTY")
	uris.Register("GET", `^/containers/(.+)/start`, AllowTrue, "container_start", "start", "Start one or more stopped containers")
	uris.Register("GET", `^/containers/(.+)/stop`, AllowTrue, "container_stop", "stop", "Stop a running container")
	uris.Register("GET", `^/containers/(.+)/restart`, AllowTrue, "container_restart", "restart", "Restart a container")
	uris.Register("GET", `^/containers/(.+)/kill`, AllowTrue, "container_kill", "kill", "Kill a running container")
	uris.Register("GET", `^/containers/(.+)/update`, AllowTrue, "container_update", "update", "Update configuration of one or more containers")
	uris.Register("GET", `^/containers/(.+)/rename`, AllowTrue, "container_rename", "rename", "Rename a container")
	uris.Register("GET", `^/containers/(.+)/pause`, AllowTrue, "container_pause", "pause", "Pause all processes within a container")
	uris.Register("GET", `^/containers/(.+)/unpause`, AllowTrue, "container_unpause", "unpause", "Unpause all processes within a container")
	uris.Register("GET", `^/containers/(.+)/attach/ws`, AllowTrue, "container_attach_ws", "attach_ws", "Attach to a running container (websocket)")
	uris.Register("GET", `^/containers/(.+)/attach`, AllowTrue, "container_attach", "attach", "Attach to a running container")
	uris.Register("GET", `^/containers/(.+)/wait`, AllowTrue, "container_wait", "wait", "Block until a container stops, then print its exit code")
